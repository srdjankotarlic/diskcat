...
A free, offline tool for video & production people to catalog which storage drive holds the footage from each shoot. No signup, no installation headaches, and your data stays with you.
...
- **Make it an app** → in Chrome/Edge on **Windows or Mac**, click **“Install app”** in the address bar; on a **phone**, use **Add to Home Screen**. One click, no installer, works offline.
...
No accounts. No setup. Nothing to configure.
...
- **Planned (optional “cloud mode”):** accounts + a free backend so a whole **team edits the same archive live** (like a shared spreadsheet). See *Roadmap*.

## Your data & privacy
Everything is stored **in your browser only** — nothing is uploaded anywhere. Click **Export** now and then to keep a backup file. To move to another device: **Export** on one, **Import** on the other.

## Roadmap (planned)
- ☁️ Optional accounts + **live shared/team archives** (cloud mode)
- 🌍 Multiple languages
...
DiskCat is **one `index.html` file** — plain HTML / CSS / JavaScript, **no build step, no dependencies**. So it's easy to change:
